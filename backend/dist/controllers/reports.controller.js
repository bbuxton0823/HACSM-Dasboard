...
exports.exportReportAsPdf = exports.streamReport = exports.getBudgetForecast = exports.getVoucherTypeReport = exports.getExecutiveSummary = void 0;
...
const mockUtilizationData = [
    {
        ...
        voucherType: HCVUtilization_1.VoucherType.TENANT_BASED,
    ...
    },
    {
        ...
        voucherType: HCVUtilization_1.VoucherType.PROJECT_BASED,
    ...
    },
    {
        ...
        voucherType: HCVUtilization_1.VoucherType.MTW_FLEXIBLE,
    ...
    },
    {
        id: '4',
        reportingDate: new Date('2024-03-01'),
        voucherType: HCVUtilization_1.VoucherType.SPECIAL_PURPOSE,
...
const getUtilizationData = async (startDate, endDate) => {
    try {
        ...
        const hcvUtilizationRepo = database_1.AppDataSource.getRepository(HCVUtilization_1.HCVUtilization);
        // Get all utilization data within date range
        const utilizationData = await hcvUtilizationRepo.find({
            where: {
                reportingDate: (0, typeorm_1.Between)(startDate, endDate)
            },
            order: {
                reportingDate: 'ASC'
            }
...
const getExecutiveSummary = async (req, res) => {
    try {
        ...
        const summary = await (0, openai_service_1.generateExecutiveSummary)(utilizationData);
        res.status(200).json({
 ...
 */
const getVoucherTypeReport = async (req, res) => {
    try {
        const { voucherType } = req.params;
        // Validate voucher type
        if (!Object.values(HCVUtilization_1.VoucherType).includes(voucherType)) {
        ...
        const utilizationData = allUtilizationData.filter(data => data.voucherType === voucherType);
        if (!utilizationData.length) {
        ...
        const report = await (0, openai_service_1.generateVoucherTypeReport)(voucherType, utilizationData);
        res.status(200).json({
...
exports.getVoucherTypeReport = getVoucherTypeReport;
/**
...
const getBudgetForecast = async (req, res) => {
    try {
        ...
        const forecastMonths = req.query.months ? parseInt(req.query.months) : 6;
        ...
        const forecast = await (0, openai_service_1.generateBudgetForecast)(utilizationData, forecastMonths);
        res.status(200).json({
...
exports.getBudgetForecast = getBudgetForecast;
/**
...
const generateMockData = (startDate, endDate) => {
    ...
    const voucherTypes = Object.values(HCVUtilization_1.VoucherType);
 ...
 */
const generateExecutiveSummaryText = (data, voucherType) => {
 ...
 */
const generateVoucherAnalysisText = (data, voucherType) => {
 ...
 */
const generateBudgetForecastText = (data, forecastMonths) => {
...
Forecast Summary:
1. Projected HAP Expenses: $${(calculateTotalHapExpenses(data) * (forecastMonths || 12) / (data.length / Object.values(HCVUtilization_1.VoucherType).length)).toLocaleString()}
 ...
 */
const proceedWithReportGeneration = (reportType, utilizationData, voucherType, forecastMonths, styleTemplate, customPrompt, formatting, res) => {
    ...
    try {
        ...
        if (reportType === 'executive_summary') {
            (0, openai_service_1.generateStreamingReport)('executive_summary', utilizationData, {
            ...
            (0, openai_service_1.generateStreamingReport)('voucher_analysis', utilizationData.filter(item => item.voucherType === voucherType), {
                voucherType,
                styleContent: styleTemplate || '',
                formatting
            }, onProgress, onComplete);
            ...
            (0, openai_service_1.generateStreamingReport)('budget_forecast', utilizationData, {
            ...
            (0, openai_service_1.generateCustomReport)(customPrompt, utilizationData, {
                styleContent: styleTemplate || '',
                formatting
            }, onProgress, onComplete);
        }
 ...
 * @param res Express response
 */
const streamReport = async (req, res) => {
    try {
        ...
        const { reportType, timeframe, voucherType, forecastMonths, styleTemplate, customPrompt, formatting } = options;
        // Validate required parameters
        if (!reportType || !(timeframe === null || timeframe === void 0 ? void 0 : timeframe.startDate) || !(timeframe === null || timeframe === void 0 ? void 0 : timeframe.endDate)) {
...
const exportReportAsPdf = async (req, res) => {
    try {
        ...
        for (let i = 0; i < pageCount; i++) {
            ...
            // Add footer text
            doc.fontSize(10)
                .text(`Page ${i + 1} of ${pageCount} | Housing Authority Dashboard | ${new Date().getFullYear()}`, 0, doc.page.height - 50, { align: 'center' });
            // Restore margins
...
exports.exportReportAsPdf = exportReportAsPdf;
